...
import type { PercentageCalcLabels } from "@/lib/tr-calculators";

import { CalculatorShell } from "@/components/calculators/calculator-shell";

type Mode = "percentOf" | "ratio" | "adjust";

type PercentageCalcToolProps = {
  locale: Locale;
  labels: PercentageCalcLabels;
};

export function PercentageCalcTool({ locale, labels }: PercentageCalcToolProps) {
  ...
  function formatExplanation(template: string, values: Record<"x" | "y" | "value", string>) {
    return template
  ...
  function handleCalculate() {
    ...
    if (mode === "percentOf") {
      value = (x * y) / 100;
      explanation = formatExplanation(labels.explanations.percentOf, {
        x: formatLocalizedNumber(locale, x),
        y: formatLocalizedNumber(locale, y),
        value: formatLocalizedNumber(locale, value),
      });
    } else if (mode === "ratio") {
      value = (x / y) * 100;
      explanation = formatExplanation(labels.explanations.ratio, {
        x: formatLocalizedNumber(locale, x),
        y: formatLocalizedNumber(locale, y),
        value: formatLocalizedNumber(locale, value, { maximumFractionDigits: 2 }),
      });
    } else {
      value = operation === "add" ? x * (1 + y / 100) : x * (1 - y / 100);
      explanation =
        operation === "add"
          ? formatExplanation(labels.explanations.add, {
              x: formatLocalizedNumber(locale, x),
              y: formatLocalizedNumber(locale, y),
              value: formatLocalizedNumber(locale, value),
            })
          : formatExplanation(labels.explanations.subtract, {
              x: formatLocalizedNumber(locale, x),
              y: formatLocalizedNumber(locale, y),
              value: formatLocalizedNumber(locale, value),
            });
    ...
    <CalculatorShell
      form={
        <div className="space-y-5">
          ...
          <label className="space-y-3">
            <span className="text-sm font-medium text-[color:var(--brand-text-primary)]">
              {mode === "percentOf" ? labels.percentageLabel : labels.yLabel}
            </span>
            <input
              value={yValue}
          ...
          {mode === "adjust" ? (
            <label className="space-y-3">
              <span className="text-sm font-medium text-[color:var(--brand-text-primary)]">{labels.operationLabel}</span>
              <select
                value={operation}
      ...
      result={
        <div className="space-y-4">
          <div className="rounded-[24px] border border-[color:var(--brand-border)] bg-[color:var(--brand-surface)] p-5">
            {result ? (
              <>
                ...
                <p className="mt-4 text-4xl font-bold text-[color:var(--brand-text-primary)]">{formatLocalizedNumber(locale, result.value, { maximumFractionDigits: 2 })}</p>
              </>
            ) : (
          ...
          {result ? (
            <div className="rounded-[24px] border border-[color:var(--brand-border)] bg-[color:var(--brand-surface)] p-5">
              <p className="text-[11px] font-semibold uppercase tracking-[0.06em] text-[color:var(--brand-badge-text)]">{labels.explanation}</p>
